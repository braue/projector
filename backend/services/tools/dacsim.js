...
// DACSIMCONVERT app. One Generate runs staging + conversion: staging copies
...
// package's own warning scaffolds, narration streams into the job log).
...
class DacsimService {
  ...
  /**
   * The GENERATED simulator folders in a run (remote + master) — the DAC
   * inputs came from the project and stay in the run only for reference.
   * Prefix-matched: a large scheme can split into "X_REMOTE", "X_REMOTE_2", …
   * Recomputed from the run's own settings.json so generate and save agree.
   */
  async #simDirs(runId) {
    const dir = await this.workspace.runDir('dacsim', runId);
    const staged = parseSchemes(
      (await this.workspace.readFile('dacsim', runId, 'settings.json')).toString(),
    );
    const prefixes = [...new Set(staged
  ...
  /**
   * Stage + convert as one job. Nothing lands in the project here — the
   * generated simulators stay in the run (the ZIP report) until save().
   */
  async generate(files, payload) {
    ...
    const job = this.jobs.start(`DAC SIM generate: ${staged.length} scheme(s)`, async (handle) => {
      ...
      return { run: runId, ...result, files: zipped, reports };
    });
    return { job: job.id, run: runId };
  }

  /**
   * The explicit "Save to project": land a finished run's simulator
   * projects in the given project's tree as versioned .rtac entries under
   * one top-level folder the user can rearrange from. The ZIP is download
   * bait only — it never enters the tree.
   */
  async save(files, runId) {
    const { dir, names } = await this.#simDirs(runId);
    if (!names.length) {
      throw httpError(404, 'this run has no generated simulator projects');
    }
